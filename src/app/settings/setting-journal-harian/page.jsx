...
const Page = () => {
    ...
    const [jumlahReminder, setJumlahReminder] = useState(0)
    ...
    const [startBackdate, setStartBackdate] = useState(null)
    const [endBackdate, setEndBackdate] = useState(null)
    ...
    const handleGetSettingJournal = useCallback(async () => {
        ...
        if (response?.data?.data) {
            ...
            setJamNotifikasiPertama(res?.jurnal_hour_1 || '07:00')
            setTitle1(res?.jurnal_title_1 || 'Halo {nama}, Saatnya mengisi jurnal')
            setDescription1(res?.jurnal_description_1 || 'Silahkan mengisi jurnal harian anda')
            if (res.jurnal_hour_2) {
                setJumlahReminder(2)
            }
            setJamNotifikasiKedua(res?.jurnal_hour_2 || '09:00')
            setTitle2(res?.jurnal_title_2 || 'Halo {nama}, Jurnal harian anda belum terisi')
            setDescription2(res?.jurnal_description_2 || 'Jurnal harian anda belum terisi, silahkan segera mengisi jurnal')
        }
        else {
            setOpenSnackbar({
                open: true,
    ...
    const handleUpdateSettingJournal = useCallback(async () => {
        ...
        const body = {
            ...
            jurnal_title_1: jumlahReminder > 0 ? title1 : '',
            jurnal_title_2: jumlahReminder > 1 ? title2 : '',
            jurnal_description_1: jumlahReminder > 0 ? description1 : '',
            jurnal_description_2: jumlahReminder > 1 ? Description2 : '',
            jurnal_hour_1: jumlahReminder > 0 ? jamNotifikasiPertama : '',
        ...
        <Layout>
            <Container>
                <div className="flex items-center justify-between gap-3">
                    ...
                    <Switch
                        onChange={() => {
                            setApprovalHierarchy(prev => !prev)
                            // handleUpdateSettingJournal()
                        }}
            ...
            <Container>
                <div className="flex items-center justify-between gap-2">
                    <h3 className="font-semibold">Pengisian Jurnal Tanggal Mundur</h3>
                    <Switch
                        onChange={() => {
                            setCheckedBackDate(prev => !prev)
                            setEndBackdate(null)
                            setStartBackdate(null)
                        }}
                        checked={checkedBackDate}
                    />
                </div>
                {checkedBackDate && (
                    <>
                            ...
                            <LocalizationProvider dateAdapter={AdapterDayjs}>
                                <div className="flex items-center gap-5">
                                    <DatePicker
                                        ...
                                        format='DD MMM YYYY'
                                        maxDate={endBackdate ? dayjs(endBackdate) : null}
                                    />
                        ...
                        <FormControlLabel
                            control={
                                <Checkbox
                                    ...
                                    onChange={(e) => {
                                        setEndBackdate(null)
                                        setCheckedBatasBackDate(e.target.checked)
                                    }}
